...
│                         User Layer                               │
│                                                                  │
│   Super+R          Super+Shift+M       Vicinae scripts           │
│   (Cinnamon)       (Cinnamon)          (4 entries)               │
│      │                  │                    │                   │
│      ▼                  ▼                    ▼                   │
│   bin/gravar       bin/marcar        recordo --toggle/etc        │
│      │                  │                    │                   │
│      └──────────────────┴────────────────────┘                   │
│                         │                                        │
│                         ▼                                        │
...
│   │ handler     │  │  loop (2s)   │  │  loop (5s)        │       │
│   │             │  │              │  │                   │       │
│   │ toggle/start│  │  hard-cap 4h │  │  pactl source-    │       │
│   │ stop/mark/  │  │  silence 10m │  │  outputs polling  │       │
│   │ status/quit │  │  remind 15m  │  │  + min_duration   │       │
│   │             │  │  seg cycle   │  │  + quiet_period   │       │
│   └──────┬──────┘  └──────┬───────┘  └─────────┬─────────┘       │
│          │                │                    │                 │
│          └────────────────┴────────────────────┘                 │
│                           │                                      │
...
└──────────────────────────┼───────────────────────────────────────┘
                           ...
                           ▼
┌──────────────────────────────────────────────────────────────────┐
│                  post_pipeline (thread)                          │
│                                                                  │
│   1. mkdir ~/Notas/<YYYY-MM-DD>_<safe_subject>/                  │
│   2. mv <subject>_<id>.opus → audio.opus                         │
│   3. write nota.md (frontmatter + marks + placeholder)           │
│   4. spawn thread: faster-whisper                                │
│      - ensure_whisper_installed() (lazy)                         │
...
`auto-detect.json` default `enabled: false`. Quando ligado:

1. Polling `pactl list source-outputs` a cada 5s
2. Match `app_name` OU `binary` contra lista permitida (substring, case-insensitive)
3. Bloqueia `deny_apps`
4. Requer mic ativo por ≥8s (filtra picos espúrios)
5. Quiet period 5min pós-stop manual (evita re-trigger)

Guardrails contra falso-positivo são fundamentais — auto-iniciar gravação não-desejada quebra confiança no sistema.
