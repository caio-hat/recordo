...
def render_view(rec: Recorder, status: str, total: float) -> Panel:
    ...
    head.add_row(
        Text.assemble(("Assunto: ", "bold"), st.subject, "  ",
                      ("ID: ", "dim"), st.session_id),
    ...
    for seg in st.segments[-12:]:
        ...
        segs_tbl.add_row(str(seg.index), seg.started_at[-8:],
                         f"{seg.duration:.1f}", size_h, seg.status)

    if rec.recording:
        elapsed = time.monotonic() - rec.seg_start_mono
        bar = Text(
            f"  ● gravando seg{len(st.segments):03d}  "
            f"({elapsed:0.1f}s / {rec.max_segment}s)",
            style="bold red")
    else:
...
def run_tui(rec: Recorder) -> None:
    ...
    with KeyReader() as kb, Live(render_view(rec, status, total),
...
def run_plain(rec: Recorder) -> None:
    ...
    while True:
        ...
        if cmd == "r":
            ...
            seg = rec.stop_segment()
            print(f"segmento {seg.index} {seg.status} dur={seg.duration:.1f}s"
                  if seg else "nada para pausar")
        elif cmd == "q":
            break
